...
```
InputController:GetPressed(action: string): boolean
InputController:GetAnalog(action: string): Vector2
InputController:GetAxis(action: string): number
```
<br>
_Use cases_<br>
- GetPressed:
  Whether or not an action is pressed (true or false)<br>
- GetAnalog:
  Return a bidirectional action's value (like Joystick or WASD) <br>
- GetAxis:
  Return the value of an action (key pressed 0 OR 1, trigger pulled any value 0 TO 1)<br>
<br>
...
<br><br>
Custom GUI generated from the config seen above. Custom analog inputs included, additionally the "AuxAction" allows for a custom action to be set when the player drags the virtual joystick far beyond the boundary of the image. In this case (and in most likely cases) this will be "sprint".<br><br>
_Mobile Layout Pictured Below_<br><br>
